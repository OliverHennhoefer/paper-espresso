...
\begin{document}
{\href{https://example.invalid/paper}{\bfseries Sample Paper Espresso Digest}%
\hfill{\footnotesize Researcher et al., 2026}\par}
\vspace{1pt}
...
\begin{equation*}
\pemathmark{peblue}{\mathcal{L}(\theta)}=
\pemathmark{pegreen}{\frac{1}{n}\sum_{i=1}^{n}\ell(f_\theta(x_i),y_i)}+
\pemathmark{peamber}{\lambda\lVert\theta\rVert_2^2}.
\end{equation*}
The \petextmark{pegreen}{data-fit term} rewards agreement with observations; the
\petextmark{peamber}{regularizer} charges for large parameters, and their sum is
the \petextmark{peblue}{training objective}.
